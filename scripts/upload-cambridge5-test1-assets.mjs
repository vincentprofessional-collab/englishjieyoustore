...
const questionImagesDir = "/Users/shidianjin/Desktop/雅思听力/雅思听力题干图片/5";
...
async function uploadSectionAssets(sectionNo) {
  const basePath = `listening/ci5/t1/s${sectionNo}`;
  const fullAudioLocalPath = path.join(sourceDir, `5test1-section${sectionNo}.mp3`);
  const sentenceAudioDir = path.join(sourceDir, `5test1-section${sectionNo}小音频`);
  ...
  for (const fileName of sentenceFiles) {
    ...
    await uploadFile(
      ...
      `${basePath}/sentences/ci5_t1_s${sectionNo}_${sentenceNo}.mp3`,
      "audio/mpeg",
    );
  }
...
async function uploadAssets() {
  ...
  console.log("Cambridge 5 Test 1 assets uploaded.");
