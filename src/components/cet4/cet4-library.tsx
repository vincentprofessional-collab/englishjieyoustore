...
const examCopy: Record<CetExam, { label: string; code: string }> = {
  cet4: { label: "大学英语四级", code: "CET-4" },
  cet6: { label: "大学英语六级", code: "CET-6" },
};
...
export function Cet4Library({ knowledge, sets, exam = "cet4", initialEntry }: { knowledge: KnowledgeSummary[]; sets: Cet4SetSummary[]; exam?: CetExam; initialEntry?: Entry }) {
  ...
  return <section className="senior-high-page">
    <div className="senior-high-hero"><div><div className="senior-high-eyebrow">LANGUAGE EXAMS · {copy.code}</div><h1>{copy.label}学习中心</h1><p>按高考英语题组结构整理{copy.label}资料：知识点、题型训练和历年试卷分开进入；文章、题干、选项和答题区按来源题组呈现。</p></div><div className="senior-high-stats"><strong>{papers.length}</strong><span>套试卷</span><strong>{practice.reduce((sum, item) => sum + item.questionCount, 0)}</strong><span>道专项题</span></div></div>
    <nav className="senior-high-entry-tabs" aria-label={`${copy.label}资料入口`}>{(Object.keys(labels) as Entry[]).map((key) => <button className={entry === key ? "selected" : ""} key={key} onClick={() => { setEntry(key); setTopic("全部"); }} type="button">{labels[key]}<small>{key === "knowledge" ? knowledge.length : key === "practice" ? practice.length : papers.length}</small></button>)}</nav>
    {entry === "knowledge" ? <div className="senior-high-section"><h2>知识点</h2><p className="senior-high-muted">独立讲解与备考方法按听力、阅读、写作归类；知识点页不混入整套试卷题目。</p><div className="senior-high-card-grid">{knowledge.map((item) => <Link className="senior-high-topic-card" href={`/${exam}/${item.id}`} key={item.id}><strong>{item.topic}</strong><span>{item.title}</span><small>{item.excerpt || "进入知识点讲解"}</small></Link>)}</div></div> : null}
