...
export function JuniorHighDemo() {
  ...
  const [region, setRegion] = useState("天津");
  ...
  if (mode === "home") return <section className="stack junior-high-page"><div className="writing-hero-panel ielts-module-hero"><div className="writing-hero-copy"><h1>中考英语</h1><p>按地区、年份和题型进入真题模考与专项训练。</p></div></div><div className="writing-mode-panel"><div className="writing-mode-grid"><button className="writing-mode-card mock" onClick={() => openMode("mock-select")} type="button"><span>完整试卷</span><strong>真题模考</strong><p>按原卷考试时间自动计时，提交后查看答案与解析。</p><em>选择年份和地区 →</em></button><button className="writing-mode-card practice" onClick={() => openMode("practice-select")} type="button"><span>题型训练</span><strong>专项训练</strong><p>选择题型、年份和地区，默认不计时，可手动开始计时。</p><em>选择训练内容 →</em></button></div></div></section>;

  if (mode === "mock-select" || mode === "practice-select") return <section className="stack junior-high-page"><div className="junior-high-selection"><button className="junior-high-back" onClick={() => setMode("home")} type="button">← 返回中考英语</button><h1>{mode === "mock-select" ? "真题模考" : "专项训练"}</h1><div className="junior-high-selection-step"><span>1</span><div><strong>选择年份</strong><div className="junior-high-chip-row">{JUNIOR_HIGH_YEARS.map((item) => <button className={year === item ? "selected" : ""} key={item} onClick={() => setYear(item)} type="button">{item}年</button>)}</div></div></div>{mode === "practice-select" ? <div className="junior-high-selection-step"><span>2</span><div><strong>选择题型</strong><div className="junior-high-type-list">{JUNIOR_HIGH_SAMPLES.map((item) => <button className={type === item.type ? "selected" : ""} key={item.type} onClick={() => setType(item.type)} type="button">{item.label}</button>)}</div></div></div> : null}<div className="junior-high-selection-step"><span>{mode === "practice-select" ? "3" : "2"}</span><div><strong>选择地区</strong><div className="junior-high-chip-row">{JUNIOR_HIGH_REGIONS.map((item) => <button className={region === item ? "selected" : ""} key={item} onClick={() => setRegion(item)} type="button">{item}</button>)}</div></div></div><button className="junior-high-confirm" disabled={!canConfirm} onClick={() => openMode(mode === "mock-select" ? "mock" : "practice")} type="button">确认并进入</button></div></section>;

  const sample = mode === "mock" ? JUNIOR_HIGH_SAMPLES[3] : selected;
  return <section className="stack junior-high-page"><div className="junior-high-workbench"><header><div><button className="junior-high-back" onClick={() => setMode(mode === "mock" ? "mock-select" : "practice-select")} type="button">← 返回选择</button><h1>中考英语 · {mode === "mock" ? "真题模考" : "专项训练"}</h1><p>{year} 年 · {region} · {mode === "mock" ? MOCK_SAMPLE.paperLabel : sample.label}</p></div><Timer running={running} seconds={seconds} onToggle={() => setRunning(!running)} /></header><div className="junior-high-workspace-grid">{sample.passage ? <div className="junior-high-reading-pane"><span>阅读材料</span><p>{sample.passage}</p></div> : null}<div className="junior-high-question-pane"><SampleQuestion sample={sample} submitted={submitted} /></div></div><footer><span>样板题目 1 / 1</span><button className="junior-high-submit" onClick={() => { setSubmitted(true); setRunning(false); }} type="button">提交答卷</button></footer></div></section>;
}
