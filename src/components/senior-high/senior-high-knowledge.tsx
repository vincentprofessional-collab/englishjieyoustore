...
const KNOWLEDGE_PREFIX_PATTERN = /^\s*\d+\s*[.．、]\s*(?:[（(]([^）)]+)[）)]\s*)?/u;
...
function knowledgePresentation(item: SeniorHighItem) {
  const match = item.stem.match(KNOWLEDGE_PREFIX_PATTERN);
  return {
    sourceLabel: match?.[1]?.trim() || "",
    stem: match ? item.stem.slice(match[0].length).trimStart() : item.stem,
  };
}

function InlineStem({ item, onChange, stem, value }: { item: SeniorHighItem; onChange: (value: string) => void; stem: string; value: string }) {
  const blanks = stem.match(BLANK_PATTERN) || [];
  const parts = stem.split(BLANK_PATTERN);
  ...
  const input = (index: number, label: string) => <input
    aria-label={label}
    autoCapitalize="none"
    autoComplete="off"
    autoCorrect="off"
    className="senior-high-inline-answer"
    data-1p-ignore="true"
    data-form-type="other"
    data-lpignore="true"
    data-senior-high-inline-answer="true"
    key={`blank-${index}`}
    name={`senior-high-knowledge-${item.id}-${index + 1}`}
    onChange={(event) => update(index, event.target.value)}
    onKeyDown={(event) => {
      if (event.key !== "Enter" || event.nativeEvent.isComposing) return;
      event.preventDefault();
      focusNextSeniorHighInlineAnswer(event.currentTarget);
    }}
    spellCheck={false}
    type="text"
    value={values[index] || ""}
  />;
  if (blanks.length === 0) return <>{stem}{input(0, `第 ${item.question_number} 题答案`)}</>;
  return parts.flatMap((part, index) => [<Fragment key={`text-${index}`}>{part}</Fragment>, index < blanks.length ? input(index, `第 ${item.question_number} 题第 ${index + 1} 空`) : null]) as ReactNode;
}
...
export function SeniorHighKnowledge() {
  ...
  return <div className="senior-high-session">
    ...
    <div className="senior-high-question-list">{questions.map((item, index) => {
      ...
      const presentation = knowledgePresentation(item);
      return <article className="senior-high-question-card" id={item.id} key={item.id}>
        <div className="senior-high-question-meta"><span>第 {index + 1} 题</span>{presentation.sourceLabel ? <small>（{presentation.sourceLabel}）</small> : null}</div>
        <p className="senior-high-question-stem">{item.options.length ? presentation.stem : <InlineStem item={item} onChange={(next) => updateAnswer(item.id, next)} stem={presentation.stem} value={value} />}</p>
        ...
        {submitted ? <div className={`senior-high-feedback ${isCorrect ? "correct" : "incorrect"}`}><span>{isCorrect ? "✓ 正确" : "✕ 未答对"}</span><strong>答案：{item.answer}</strong>{item.analysis ? <div className="senior-high-analysis"><p>{item.analysis}</p></div> : <small>暂无解析</small>}</div> : null}
      </article>;
    })}</div>
